...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        if (okToRender && pdfDocument) {                                              // if the pdf Document has been set, then:
            console.log('useEffect 2')
            renderPage(pageNum, scale);                                        // Render the Page (pageNum is initially set to page# 1)
        }
    }, [pageNum, scale]);                                               // Removed "pdfDocument"...

    //Render the page
    const renderPage = (num, scale) =>{                                          //Function for rendering page
