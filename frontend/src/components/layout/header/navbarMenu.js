...
const NavbarMenu = () => {
    ...
    const [isActiveHomeIcon, setIsActiveHomeIcon] = useState(false)
    const homeIcon = (
        <svg width="22" height="19" viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M11.25 0.0625L0.75 7.9375H3.375V18.4375H8.625V13.1875H13.875V18.4375H19.125V7.85875L21.75 7.9375L11.25 0.0625Z" 
            fill="currentColor"
            />
        </svg>
    );
    const [isActiveDashboardIcon, setIsActiveDashboardIcon] = useState(false)
    const dashboardIcon = (
    ...
    const [isActiveAboutUsIcon, setIsActiveAboutUsIcon] = useState(false)
    const aboutUsIcon = (
        <svg width="22" height="19" viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const [isActiveSupportCenterIcon, setIsActiveSupportCenterIcon] = useState(false)
    ...
    let content = <div className={classes.buttonPadding}><GetStartedButton /></div>
    isAuthenticated 
    ? content = null 
    ...
    isAuthenticated
    ? option = (<a  href='#' onClick={LogOutHandler} className={classes.menuItem}><i className={classes.icon} onClick={LogOutHandler}>{logOutIcon}</i><span>Log in</span></a>)
    : option = (<a href='#' onClick={LogInHandler} className={classes.menuItem}><i className={classes.icon} onClick={LogInHandler}>{logInIcon}</i><span>Log in</span></a>)
            ...
            <menu className={classes.sidebar} ref={navBarRef}>
                <div className={classes.top}>
                    ...
                    <div className={classes.sidebarMenu}>
                        <a href="/home"  className={`${classes['homeIconStyle']} ${isActiveHomeIcon && classes.isActiveHomeIcon}`}><i>{homeIcon}</i><span>Home</span></a>
                        <a href='demo.html' className={`${classes['homeIconStyle']} ${isActiveDashboardIcon && classes.isActiveDashboardIcon}`}><i>{dashboardIcon}</i><span>Demo</span></a>
                    </div>
                </div>
                <div className={classes.bottom}>
                    <div className={classes.subgroup}>
                        <div className={classes.sidebarMenu}>
                            <a href='aboutus.html' className={`${classes['homeIconStyle']} ${isActiveAboutUsIcon && classes.isActiveAboutUsIcon}`}><i>{aboutUsIcon}</i><span>About Us</span></a>
                            <a href='supportcenter.html' className={`${classes['homeIconStyle']} ${isActiveSupportCenterIcon && classes.isActiveSupportCenterIcon}`}><i>{supportCenterIcon}</i><span>Support Center</span></a>
                            <a href='help.html' className={`${classes['homeIconStyle']} ${isActiveHelpIcon && classes.isActiveHelpIcon}`}><i>{helpIcon}</i><span>Help</span></a>
                        </div>
                    </div>
                    <div className={classes.break}></div>
                    <div className={classes.subgroup}>
                        <div className={classes.sidebarMenu}>
                            <a href='language.html' className={`${classes['homeIconStyle']} ${isActiveLanguageIcon && classes.isActiveLanguageIcon}`}><i>{languageIcon}</i><span>Language</span></a>
